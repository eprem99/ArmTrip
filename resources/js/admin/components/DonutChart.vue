    ...
    <div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
        <div class="mb-4 flex items-center justify-between">
            <div>
                <p class="text-xs font-medium uppercase tracking-wide text-slate-500">
                    {{ t('admin.dashboard.conversion_by_channel') }}
                </p>
                <p class="mt-1 text-sm font-medium text-slate-900">{{ t('admin.dashboard.conversion_by_channel') }}</p>
            </div>
            <select
            ...
            >
                <option value="7d">{{ t('admin.dashboard.last_7_days') }}</option>
                <option value="30d">{{ t('admin.dashboard.last_30_days') }}</option>
                <option value="90d">{{ t('admin.dashboard.last_90_days') }}</option>
            </select>
        </div>
        <div class="flex items-center gap-6">
            <div class="relative mx-auto h-40 w-40">
                ...
                >
                    <p class="text-xs font-medium text-slate-500">{{ t('admin.dashboard.total') }}</p>
                    <p class="mt-1 text-2xl font-semibold text-slate-900">140</p>
                    <p class="mt-0.5 text-[11px] text-slate-500">{{ t('admin.dashboard.orders_count') }}</p>
                </div>
            </div>
            <div class="hidden flex-1 space-y-3 text-xs text-slate-700 sm:block">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span>{{ t('admin.dashboard.online_store') }}</span>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span>{{ t('admin.dashboard.marketplace') }}</span>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span>{{ t('admin.dashboard.offline') }}</span>
                    </div>
