    ...
    <div class="space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <p class="text-sm text-slate-500">
                    <a href="/admin/rentals" class="font-medium text-blue-600 hover:text-blue-800">{{ t('admin.rentals_types.back_rentals') }}</a>
                </p>
                <h1 class="mt-2 text-xl font-semibold text-slate-900">{{ t('admin.rentals_types.title') }}</h1>
                <p class="mt-1 text-sm text-slate-500">{{ t('admin.rentals_types.description') }}</p>
            </div>
            <button
                type="button"
            ...
            >
                ...
                {{ t('admin.rentals_types.new') }}
            </button>
        </div>

        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
                <p class="text-sm text-slate-600">{{ pagination.total }} {{ t('admin.rentals_types.count') }}</p>
                <div class="relative">
                    <MagnifyingGlassIcon class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                    <input
                        v-model="searchQuery"
                        type="search"
                        :placeholder="t('admin.rentals_types.search_placeholder')"
            ...
            <AdminTable>
                <template #head>
                    <th class="px-4 py-3">{{ t('admin.rentals_types.col_name') }}</th>
                    <th class="px-4 py-3">{{ t('admin.rentals_types.col_slug') }}</th>
                    <th class="px-4 py-3">{{ t('admin.rentals_types.col_sort') }}</th>
                    <th class="px-4 py-3 text-right">{{ t('admin.rentals_types.action') }}</th>
                </template>
                <tr v-if="loading">
                    <td colspan="4" class="px-4 py-8 text-center text-slate-500">{{ t('admin.rentals_types.loading') }}</td>
                </tr>
                <tr v-else-if="error">
                ...
                <template v-else>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    ...
                                    :title="t('admin.rentals_types.edit')"
                                    @click="openEdit(row)"
                                >
                                ...
                                <button
                                    ...
                                    :title="t('admin.rentals_types.delete')"
                    ...
                    <tr v-if="rows.length === 0">
                        <td colspan="4" class="px-4 py-8 text-center text-slate-500">{{ t('admin.rentals_types.empty') }}</td>
                    </tr>
                </template>
            </AdminTable>
            ...
            >
                <p class="text-sm text-slate-600">
                    {{ pagination.from }}–{{ pagination.to }} {{ t('admin.rentals_types.pagination_of') }} {{ pagination.total }}
                </p>
            ...
            >
                ...
                <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
                    <h3 class="text-lg font-semibold text-slate-900">
                        {{ editingId ? t('admin.rentals_types.form_edit_title') : t('admin.rentals_types.form_create_title') }}
                    </h3>
                    <div class="mt-4 space-y-3">
                        <div>
                            <label class="block text-xs font-medium text-slate-600">{{ t('admin.rentals_types.form_name') }}</label>
                            <input
                                v-model="form.name"
                                type="text"
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-600">{{ t('admin.rentals_types.form_slug') }}</label>
                            <input
                                v-model="form.slug"
                                type="text"
                                class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-sm"
                                :placeholder="t('admin.rentals_types.form_slug_placeholder')"
                            />
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-600">{{ t('admin.rentals_types.form_description') }}</label>
                            <textarea
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-600">{{ t('admin.rentals_types.form_sort') }}</label>
                            <input
                    ...
                    <div class="mt-6 flex flex-wrap gap-3">
                        ...
                        <button
                            ...
                            @click="submitForm"
                        >
                            {{ saving ? t('admin.rentals_types.saving') : t('admin.rentals_types.save') }}
                        </button>
                    </div>
                </div>
            </div>
        ...
        <Teleport to="body">
            ...
            >
                ...
                <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
                    <h3 class="text-lg font-semibold text-slate-900">{{ t('admin.rentals_types.delete_confirm_title') }}</h3>
                    <p class="mt-2 text-sm text-slate-600">{{ t('admin.rentals_types.delete_confirm_message') }}</p>
                    ...
                    <div class="mt-6 flex flex-wrap gap-3">
                        <button
                            ...
                            @click="closeDelete"
                        >
                            {{ t('admin.rentals_types.delete_confirm_cancel') }}
                        </button>
                        <button
                            type="button"
                            :disabled="deleting"
                            ...
                            @click="confirmDelete"
                        >
                            {{ deleting ? t('admin.rentals_types.loading') : t('admin.rentals_types.delete_confirm_confirm') }}
                        </button>
                    </div>
                </div>
            </div>
...
async function load(page = 1) {
    ...
    try {
        ...
        const res = await axios.get('/admin/rentals/api/types', {
            params,
        ...
        error.value = e.response?.data?.message || e.message || t('admin.rentals_types.load_error');
    } finally {
        loading.value = false;
    }
}
...
async function submitForm() {
    ...
    if (!form.value.name?.trim()) {
        formError.value = t('admin.rentals_types.form_name_required');
        return;
    }
    setCsrf();
    saving.value = true;
    try {
        ...
        if (editingId.value) {
            await axios.put(`/admin/rentals/api/types/${editingId.value}`, payload, {
                headers: { Accept: 'application/json' },
            });
        } else {
            await axios.post('/admin/rentals/api/types', payload, {
                headers: { Accept: 'application/json' },
            });
        }
...
async function confirmDelete() {
    ...
    deleteError.value = '';
    try {
        const res = await axios.delete(`/admin/rentals/api/types/${rowToDelete.value.id}`, {
            validateStatus: () => true,
        });
        if (res.status === 422) {
            deleteError.value = res.data?.message || t('admin.rentals_types.delete_in_use');
            return;
        }
        if (res.status >= 200 && res.status < 300) {
            closeDelete();
            ...
            deleteError.value = res.data?.message || `HTTP ${res.status}`;
        }
    } catch (e) {
        deleteError.value = e.response?.data?.message || e.message;
    } finally {
